...
class JsonHandler:
    ...
    def create_empty_v2_structure(self) -> Dict[str, Any]:
    ...
    def validate_json(self, json_data: Dict[str, Any]) -> bool:
        try:
            required_fields = ["spec", "spec_version", "data", "metadata"]
    ...
    def parse_json(self, json_string: str) -> Optional[Dict[str, Any]]:
        try:
    ...
    def to_json_string(self, data: Dict[str, Any]) -> Optional[str]:
        try:
            return json.dumps(data, indent=2)
        except Exception as e:
            if self.logger:
                self.logger.error(f"JSON stringify error: {str(e)}")
            return None
...
class JsonViewer:
    def __init__(self, parent_frame, json_handler, logger):
        ...
        self.json_view = None  # No longer using a Tkinter Text widget
