...
Tile-based tactical combat replacing the older slot-based system. Combat takes place on the local map (15x15 grid with center at 7,7).
...
- **Grid:** 15 columns x 15 rows (square with center at 7,7), each tile has terrain type, traversability, highlights
...
interface RoomLayoutData {
  gridSize: { cols: number; rows: number };  // Matches local map grid (15x15)
...
- Mock external third-party services at app boundary (e.g., OpenAI, Codex)
